...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const labelsdefault =[
    ...
    { name: 'Belum Mengisi', color: '#ff9800' }
];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const kampusData = document.querySelector('#kampusChart').getAttribute('data-id');
  const parsedData = JSON.parse(kampusData);

  // Proses data untuk Chart.js
  const labels = parsedData.map(item => item.tracer_kuliah_kampus);
  const data = parsedData.map(item => item.total);
  ...
  const kampusChart = new Chart(ctx, {
      type: 'pie',
      data: {
          labels: labels,
          datasets: [{
      ...
      options: {
          ...
          plugins: {
              legend: {
                  display: true // Tampilkan legenda bawaan
              },
          }
      }
  });
